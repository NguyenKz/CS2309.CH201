...
def build_app(weights_fp16: Path):
    ...
    def generate_paper_candidate_batch(
        ...
        info = (
            f"{runtime_banner}  \n"
            f"**Paper demo** (chỉ prompt, self-guided mask) · "
    ...
    with gr.Blocks(
        ...
        gr.Markdown(
            "# SwiftEdit-RT — Demo one-step editing (T4)\n"
            f"**Paper demo** = chỉ upload + prompt. **ROI / tô mask** = mở rộng đề tài.  \n"
            f"Colab T4: **fp16 disk + channels_last + xFormers + EditCache** trên `{device}`.\n\n"
            f"{runtime_banner}"
        )
        with gr.Tabs():
            with gr.Tab("Paper demo (chỉ prompt)"):
                ...
                gr.Markdown(
                    "Khớp paper: **upload ảnh → source/edit prompt → 1 kết quả**. "
                    "Không tô mask (self-guided). Source prompt có thể để trống."
                )
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        paper_src = gr.Textbox(
                            ...
                            placeholder="vd: a mountain bicycle — có thể để trống",
                        )
            ...
            with gr.Tab("ROI / tô mask (mở rộng)"):
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        gr.Markdown(
                            "**Mở rộng đề tài:** tô mask → crop ROI → Hybrid blend. "
                            "Không bắt buộc theo paper."
                        )
